...
void DeleteEntrys(Table* table){
    ...
    if(in_main && table->next_local_index == 1){ // exit main function
...
void InsertTableEntryFromidList(Table *t, idList* l, const char* kind, Type* type){ // int a,b[3][2],c,d[5];
	...
	while(p != NULL){
		...
		if(t->level != 0 && strcmp(kind, "constant") != 0){ // constant no index
			tmp->local_index = p->ptr->local_index;
		}
...
idList* AddToidList(Table *table, idList* ret, const char* name, TypeDim *dim, Attribute *a){
	// a : RHS
	// dim : LHS
	if(ret == NULL){
		...
		ret->next = NULL;
		if(table->level != 0){
			ret->ptr->local_index = table->next_local_index;
			table->next_local_index ++;
		}
	}
	else{
		...
		p->next->next = NULL;
		if(table->level != 0){
			p->next->ptr->local_index = table->next_local_index;
			table->next_local_index ++;
		}
	}
...
FuncParamidList* AddToFuncParamidList(Table *table, FuncParamidList* ret, const char* name, Type *type){
	if(ret == NULL){
		ret = (FuncParamidList*)malloc(sizeof(FuncParamidList));
		strncpy(ret->id, name, 32);
		ret->type = type;
		ret->next = NULL;
		ret->local_index = table->next_local_index;
		table->next_local_index ++;
	}
	else{
		...
		p->next->type = type;
		p->next->next = NULL;
		p->next->local_index = table->next_local_index;
		table->next_local_index ++;
	}
	return ret;
}
...
Expr *FindVar(Table *t, const char *name){
	...
	/* debug appear and error 1/13 03:53
...
Expr *FindExprListEnd(ExprList *l){
	if(l == NULL){
		return NULL;
	}
	else{
