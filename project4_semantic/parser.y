...
identifier_list : identifier_list COMMA ID {idlist = AddToidList(idlist, $3, NULL, NULL);}
                | identifier_list COMMA ID ASSIGN_OP logical_expression {idlist = AddToidList(idlist, $3, NULL, BuildExprAttribute($5));}
                | identifier_list COMMA ID dim ASSIGN_OP initial_array {idlist = AddToidList(idlist, $3, $4, BuildExprListAttribute($6));}
                | identifier_list COMMA ID dim {idlist = AddToidList(idlist, $3, $4, NULL);}
                | ID dim ASSIGN_OP initial_array {idlist = AddToidList(idlist, $1, $2, BuildExprListAttribute($4));}
                | ID dim {idlist = AddToidList(idlist, $1, $2, NULL);}
                | ID ASSIGN_OP logical_expression {idlist = AddToidList(idlist, $1, NULL, BuildExprAttribute($3));}
...
const_list : const_list COMMA ID ASSIGN_OP literal_const {idlist = AddToidList(idlist, $3, NULL, BuildValueAttribute($5));}
           | ID ASSIGN_OP literal_const 
            {
              idlist = AddToidList(idlist, $1, NULL, BuildValueAttribute($3));
            }
           ;
...
simple_statement : variable_reference ASSIGN_OP logical_expression SEMICOLON { CheckType($1, $3);}
                 | PRINT logical_expression SEMICOLON {CheckPtRd($2);}
                 | READ variable_reference SEMICOLON {CheckPtRd($2);}
                 ;
conditional_statement : IF L_PAREN logical_expression R_PAREN L_BRACE { CheckBool($3); symbol_table->level ++;} var_const_stmt_list R_BRACE {PrintTable(symbol_table);DeleteEntrys(symbol_table);symbol_table->level--;} conditional_else /* new statement */
...
while_statement : WHILE L_PAREN logical_expression R_PAREN
                    L_BRACE {CheckBool($3); symbol_table->level++; in_forwhile = true;} var_const_stmt_list R_BRACE {PrintTable(symbol_table);DeleteEntrys(symbol_table);symbol_table->level--; in_forwhile = false;}
                | DO L_BRACE {symbol_table->level++; in_forwhile = true;}
                    var_const_stmt_list
                  R_BRACE {PrintTable(symbol_table);DeleteEntrys(symbol_table);symbol_table->level--;} WHILE L_PAREN logical_expression R_PAREN SEMICOLON { CheckBool($9); in_forwhile = false;}
                ;
...
for_statement : FOR L_PAREN initial_expression_list SEMICOLON control_expression_list SEMICOLON increment_expression_list R_PAREN 
                    L_BRACE {symbol_table->level++; in_forwhile = true;} var_const_stmt_list R_BRACE {PrintTable(symbol_table);DeleteEntrys(symbol_table);symbol_table->level--; in_forwhile = false;}
              ;
...
initial_expression : initial_expression COMMA variable_reference ASSIGN_OP logical_expression { CheckType($3, $5);}
                   | initial_expression COMMA logical_expression
                   | logical_expression
                   | variable_reference ASSIGN_OP logical_expression {CheckType($1, $3);}

control_expression_list : control_expression
                        |
                        ;

control_expression : control_expression COMMA variable_reference ASSIGN_OP logical_expression {CheckType($3, $5); }
...
increment_expression : increment_expression COMMA variable_reference ASSIGN_OP logical_expression {CheckType($3, $5);}
                            ...
                            { 
                              if(FindEntryInGlobal(symbol_table, $1) == NULL){
                                ...
                                error = true;
                                  ...
                                  if(flag){
                                    CheckFuncParam(FindEntryInGlobal(symbol_table, $1)->attr->attrparam, BuildExprListAttribute($3)->attrparam);
                                  }
...
jump_statement : CONTINUE SEMICOLON { CheckContinueBreak(in_forwhile); last_return = false;}
               ...
               | RETURN logical_expression SEMICOLON { CheckFuncReturn(return_type, $2); last_return = true;}
               ;
...
logical_expression : logical_expression OR_OP logical_term { $$ = Logic($1, $3, $2);}
                   | logical_term {$$ = $1;}
                   ;

logical_term : logical_term AND_OP logical_factor { $$ = Logic($1, $3, $2);}
             | logical_factor {$$ = $1;}
             ;

logical_factor : NOT_OP logical_factor { $$ = Not($2, $1);}
               | relation_expression {$$ = $1;}
               ;

relation_expression : relation_expression relation_operator arithmetic_expression {$$ = Relation($1,$3,$2);}
...
arithmetic_expression : arithmetic_expression ADD_OP term { $$ = Arith($1, $3, $2);}
                      | arithmetic_expression SUB_OP term { $$ = Arith($1, $3, $2);}
                      | term {$$ = $1;}
                      ;

term : term MUL_OP factor { $$ = Arith($1, $3, $2);}
     | term DIV_OP factor { $$ = Arith($1, $3, $2);}
     | term MOD_OP factor { $$ = Mod($1, $3, $2);}
     | factor {$$ = $1;}
     ;

factor : SUB_OP factor { $$ = Negative($2);}
       | literal_const { $$ = ConstToExpr($1);}
       | variable_reference { $$ = $1;}
       | L_PAREN logical_expression R_PAREN { $$ = $2;}
       | ID L_PAREN logical_expression_list R_PAREN 
          {
            if($3 == NULL){ // parameter is wrong not declared
              $$ = NULL;
            }
            else{
              Expr *e;
              if(FindEntryInGlobal(symbol_table, $1) == NULL){
                ...
                e = Function_invoke($1, $3, symbol_table);
              }
              $$ = e;
            }
